...
    console.log(`✅ Mensagem da fila enviada para ${job.recipient}`);
  } catch (e) {
    console.error(`❌ Falha ao enviar mensagem da fila para ${job.recipient}:`, e);
  }
  const delay = Math.floor(Math.random() * (20000 - 7000 + 1)) + 7000;
  console.log(`[Anti-Spam] Próxima mensagem em ${delay / 1000}s...`);
  setTimeout(() => {
...
        console.log("--------------------------------------------------");
        console.log(`➡️  Escaneie em: ${publicUrl}/qr`);
        console.log("--------------------------------------------------\n");
      }
    }
    if (connection === "close") {
...
  const SECRET_KEY = process.env.BOT_API_SECRET;
  ...
  app.post('/boas-vindas-individual', async (req, res) => {
    try {
      if (req.headers['x-secret-key'] !== SECRET_KEY) {
        return res.status(403).send('Acesso negado.');
      }
      ...
      if (!alunoId) {
        return res.status(400).send('ID do aluno ausente.');
      }
      ...
      if (error || !aluno) {
        console.error(`[Boas-Vindas Auto] Erro ao buscar aluno ${alunoId}:`, error);
        return res.status(404).send('Aluno não encontrado para boas-vindas.');
      }
      ...
      notificationQueue.push({ recipient: numeroResponsavel, message: mensagem });
      console.log(`[Boas-Vindas Auto] Mensagem para responsável por ${aluno.nome} adicionada à fila.`);
      
      processQueue(sock);

      res.status(200).send('Mensagem de boas-vindas individual enfileirada.');

    } catch (err) {
      console.error("❌ Erro no endpoint /boas-vindas-individual:", err);
