...
async function handleCadastroResponsavel(sock, msg) {
  try {
    ...
    if (!estado && body.toLowerCase().includes("cadastrar telefone")) {
    ...
    } else if (estado && body.toLowerCase().includes("cadastrar telefone")) {
      await sock.sendMessage(sender, { text: "⚠️ Você já está em um fluxo de cadastro. Digite 'cancelar' para recomeçar." });
      return;
    }
